...
import { NewDropsSkeleton } from '@/src/components/skeletons';
import type { NewDropCard } from '@/src/utils/discoveryViewModels';
...
export interface NewDropsSectionProps {
  ...
  loading: boolean;
  error?: boolean;
  onItemPress: (item: NewDropCard) => void;
  onImpression?: (items: NewDropCard[]) => void;
  testID?: string;
}

const NewDropsSection: React.FC<NewDropsSectionProps> = ({
  data,
  loading,
  error = false,
  onItemPress,
  ...
  if (state === 'loading') {
    return <NewDropsSkeleton />;
  }

  if (state === 'error') {
    return (
      <View style={styles.emptyContainer} testID={testID ? `${testID}-error` : undefined}>
        <Text style={styles.emptyText}>{t('home.discovery.newDrops.error')}</Text>
      </View>
    );
  }
  ...
  if (state === 'empty') {
    return (
      <View style={styles.emptyContainer} testID={testID ? `${testID}-empty` : undefined}>
        <Text style={styles.emptyText}>{t('home.discovery.newDrops.empty')}</Text>
      </View>
    );
  }
